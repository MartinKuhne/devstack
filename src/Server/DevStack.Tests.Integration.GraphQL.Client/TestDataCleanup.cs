...
public class TestDataCleanup : ITestDataCleanup
{
    public async Task CleanupAsync(DevStackDbContext context)
    {
        await context.Database.ExecuteSqlRawAsync(
            "TRUNCATE TABLE \"Projects\", \"Items\", \"Tasks\", \"ModelConfigurations\", \"WorkflowRuns\", \"AuditEvents\" RESTART IDENTITY CASCADE");
    }
    ...
    public async Task VerifyCleanupAsync(DevStackDbContext context)
    {
        ...
        if (projectCount > 0 || itemCount > 0 || taskCount > 0 || 
            modelConfigCount > 0 || workflowRunCount > 0 || 
            auditEventCount > 0)
        {
            throw new InvalidOperationException(
                $"Data cleanup failed. Remaining: Projects={projectCount}, " +
                $"Items={itemCount}, Tasks={taskCount}, " +
