...
test.describe('Agent Task CRUD', () => {
    ...
    test('agent task detail - back to list navigation', async ({ page }) => {
        ...
        if (hasError) {
            ...
            const hasDashboardOrTasks = await page.getByRole('heading', { name: /Dashboard|Agent Tasks/ }).isVisible().catch(() => false);
            if (hasDashboardOrTasks) {
                await expect(page.getByRole('heading', { name: /Dashboard|Agent Tasks/ })).toBeVisible();
            }
        }
    });
    ...
    test('agent tasks table shows Agent column', async ({ page }) => {
        ...
        const hasAgentHeader = await page.getByRole('columnheader', { name: 'Agent' }).isVisible().catch(() => false);
        if (hasAgentHeader) {
    ...
    test('agent tasks table shows Tokens column', async ({ page }) => {
        ...
        const hasTokensHeader = await page.getByRole('columnheader', { name: 'Tokens' }).isVisible().catch(() => false);
        if (hasTokensHeader) {
